...
export default function TravelMapApp() {
  ...
  useEffect(() => {
    ...
    // ★★★ 取得當前網址，用於標頭顯示 ★★★
    // 修改：直接使用固定的網址字串 travel-map-delta.vercel.app，不帶參數
    const appUrl = `travel-map-delta.vercel.app`;
    ...
    if (hasData && bounds.isValid()) {
        ...
        // 先前為 [50, 50] (電腦) / [20, 20] (手機)，現在改得更小以最大化顯示
        const padding = isPortrait ? [10, 10] : [30, 30]; 
