...
export default function TravelMapApp() {
  ...
  const fetchCitiesForCountry = useCallback(async (country, type) => {
    ...
    if (type === 'origin') { setCities = setOriginCities; setLoading = setIsLoadingOriginCities; setManual = setIsOriginManual; } 
    else if (type === 'dest') { setCities = setDestCities; setLoading = setIsLoadingDestCities; setManual = setIsDestManual; } 
    else if (type === 'transit') { setCities = setTransitCities; setLoading = setIsLoadingTransitCities; setManual = setIsTransitManual; }

    setLoading(true);
    setManual(false); 

    if (PREDEFINED_CITIES[country]) {
        const processedCities = PREDEFINED_CITIES[country].map(city => ({ value: getDisplayCityName(city), label: getDisplayCityName(city), original: city }));
    ...
    try {
      const response = await fetch('https://countriesnow.space/api/v0.1/countries/cities', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ country: country }) });
      const data = await response.json();
      if (!data.error && data.data && data.data.length > 0) {
        const processedCities = data.data.map(city => ({ value: getDisplayCityName(city), label: getDisplayCityName(city), original: city }));
        processedCities.sort((a, b) => a.label.localeCompare(b.label));
        setCities(processedCities);
      } else { setCities([]); setManual(true); }
    } catch (error) { setCities([]); setManual(true); } finally { setLoading(false); }
  }, []);
  ...
  const openModal = useCallback((countryName = '', tripToEdit = null) => {
    try {
        ...
        if (tripToEdit) {
            ...
            if (tripToEdit.transitCountry) { fetchCitiesForCountry(tripToEdit.transitCountry, 'transit'); }
        } else {
            setEditingId(null);
            const currentTrips = latestDataRef.current?.trips || [];
            let initOriginCountry = '', initOriginCity = '', initOriginLat = null, initOriginLng = null;
            ...
            setFormData({
            originCountry: initOriginCountry || '', originCity: initOriginCity || '', originLat: initOriginLat, originLng: initOriginLng,
            destCountry: initOriginCountry || '', destCity: '', destLat: null, destLng: null,
            transitCountry: '', transitCity: '', transitLat: null, transitLng: null,
            dateStart: '', timeStart: '', dateEnd: '', timeEnd: '',
            transport: 'plane', cost: '', currency: 'EUR',
            transportNumber: '', seatNumber: '', seatType: '', notes: '',
            ...
            } else { setOriginCities([]); setDestCities([]); }
            setTransitCities([]);
        }
        setIsModalOpen(true);
    } catch (err) {
  ...
  useEffect(() => { if (!loading && mapLoaded) { renderMapLayers(trips); } }, [trips, loading, mapLoaded, renderMapLayers]);
  ...
  useEffect(() => {
    ...
    map.on('click', (e) => {
      if (pickingLocationMode.current) {
        ...
        let fieldLat = 'originLat', fieldLng = 'originLng';
            ...
            layer.on({
              ...
              click: (e) => {
                if (pickingLocationMode.current) {
                  ...
                  let fieldCountry = 'originCountry', fieldCity = 'originCity';
    ...
    const handleMapClick = () => {
      setTimeout(() => {
         if (isPickingMode) {
             ...
             setIsModalOpen(true); 
             const style = document.getElementById('map-cursor-style');
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    if (!currentMapId) { alert("錯誤：找不到 Map ID，無法儲存。請嘗試重新登入。"); return; }
    setIsSaving(true);
    ...
    try {
        if (transportType && transportType.useRoute && formData.originLat && formData.originLng && formData.destLat && formData.destLng) {
            finalRoutePath = await fetchRoutePath(formData.originLat, formData.originLng, formData.destLat, formData.destLng, formData.transitLat, formData.transitLng);
        }
    } catch(err) { console.warn("路徑抓取失敗，將使用直線代替", err); }
    
    const finalData = { ...formData, routePath: finalRoutePath ? JSON.stringify(finalRoutePath) : null };
    try {
      if (editingId) { await updateDoc(doc(db, 'artifacts', appId, 'users', currentMapId, 'travel_trips', editingId), { ...finalData, updatedAt: serverTimestamp() }); } 
      else { await addDoc(collection(db, 'artifacts', appId, 'users', currentMapId, 'travel_trips'), { ...finalData, createdAt: serverTimestamp() }); }
      setIsModalOpen(false);
      if (mapInstanceRef.current && pickerMarkerRef.current) { mapInstanceRef.current.removeLayer(pickerMarkerRef.current); pickerMarkerRef.current = null; }
    } catch (err) { console.error("Error saving trip:", err); alert("儲存失敗，請檢查網路連線或權限。"); } finally { setIsSaving(false); }
  ...
  const renderCityInput = (type) => {
    ...
    if (type === 'origin') { cities = originCities; isLoading = isLoadingOriginCities; isManual = isOriginManual; setManual = setIsOriginManual; fieldCountry = 'originCountry'; fieldCity = 'originCity'; fieldLat = 'originLat'; fieldLng = 'originLng'; label = '出發城市/地點'; placeholder = '例如: 台北'; } 
    else if (type === 'dest') { cities = destCities; isLoading = isLoadingDestCities; isManual = isDestManual; setManual = setIsDestManual; fieldCountry = 'destCountry'; fieldCity = 'destCity'; fieldLat = 'destLat'; fieldLng = 'destLng'; label = '抵達城市/地點'; placeholder = '例如: 東京'; } 
